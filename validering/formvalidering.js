...
function validering(){ //Dette er den store funktion der håndterer validering af alle felter!
	...
	//Her skal være validering af nogle felter der er sprunget over.
	
	
	//validering af email. Vi anvender et regulært udtryk
	var emailRE = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b/i
	//OBS: Der findes ikke et perfekt udtryk for email! se link http://www.regular-expressions.info/email.html

	if (!emailRE.test(document.tilmelding.email.value)){
		visfejltekst('Emailadressen er ikke gyldig!');
...
function showContent(){	//Viser bare at det er gået godt, samt de bestilte gaver.
	...
	vissuccestekst("Det er gået godt! Vest: " + vestvalgt + ". Dunk: " + dunkvalgt + ". Fotovalgt: " + fotovalgt +".")
	return false;
}